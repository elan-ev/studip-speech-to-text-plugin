...
const inputFileRef = computed(() => props.job?.["input-file-ref"] ?? null);
...
<template>
  <tr>
    ...
    <td v-if="Object.keys(job['output-file-refs']).length">
      <div>
        <OutputBadge v-for="(fileRef, ext) in job['output-file-refs']" :file="fileRef" />
      </div>
    </td>
    <td v-else>
